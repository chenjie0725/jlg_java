...
public class SnowflakeIDGenImpl implements IDGen {
    ...
    public synchronized Result get(String key) {
        ...
        if (timestamp < lastTimestamp) {
            ...
            if (offset <= 5) {
                try {
                    ...
                    timestamp = timeGen();
                    if (timestamp < lastTimestamp) {
                        return new Result(Status.EXCEPTION, -1);
                    }
                } catch (InterruptedException e) {
                    log.error("wait interrupted");
                    return new Result(Status.EXCEPTION, -2);
                }
            } else {
                return new Result(Status.EXCEPTION, -3);
            }
        }
        ...
        return new Result(String.valueOf(id), Status.SUCCESS, 1);
